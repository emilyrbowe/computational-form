//Code modified from kusakari's sketch: https://www.openprocessing.org/sketch/939203
let message = ["C", "O", "M", "P", "F", "O", "R", "M"];
let _aryObjects = [];

function preload() {
  font = loadFont('GT-Cinetype-Trial-Mono.otf');
}
...
function setup() {
  ...
  noStroke();
  let objectNum = 4;
  let rMax = width / 1.7 * random(0.2, 1);
  let yGap = height / objectNum / 6;
  let divideNum = 16;
  ...
  for (let index = 0; index < message.length; index++) {
    ...
    for (let i = 0; i < objectNum; i++) {
      let myArc = new Arc(num, rMax, i, objectNum, yGap,
        divideNum, noiseSeedRad + i * noiseStepObj, noiseSeedAng + i *
        noiseStepObj, noiseSeedT + i * noiseStepObj,
        noiseStepRad, noiseStepAng, noiseSpeedRad, noiseSpeedAng, noiseSpeedT,
        rOffset);
...
class Arc {
  constructor(num, rMax, obj_i, objectNum, yGap,
    divideNum, noiseSeedRad, noiseSeedAng, noiseSeedT,
    noiseStepRad, noiseStepAng, noiseSpeedRad, noiseSpeedAng, noiseSpeedT,
    rOffset) {
    this.centX = (2 * num + 1) * width / 10 + num * width / 10;
  ...
  drawMe() {
    ...
    for (let i = 0; i < this.divideNum + 3; i++) {
      let tgt_i = i;
      if (tgt_i >= this.divideNum) {
        tgt_i -= this.divideNum;
      }
      currentNoiseVal += noise(this.noiseSeedAng + tgt_i * this.noiseStepAng) *
        3;
      let maxAng = 2 * PI * currentNoiseVal / sumNoiseVal + 2 * PI;
      let rx = this.rMax * (noise(this.noiseSeedRad + tgt_i * this.noiseStepRad,
        this.noiseSeedT) + this.rOffset) * * d * cos(maxAng);
      let ry = this.rMax * (noise(this.noiseSeedRad + tgt_i * this.noiseStepRad,
        this.noiseSeedT) + this.rOffset) * * d * sin(maxAng) / 2.5;
      if (tgt_i == 0) {
        rx_0 = rx;
        ry_0 = ry;
      ...
      // font.textToPoints(message[0], this.centX, this.centY, 16)
    }
    endShape();
  }
}
